...
export default class Month extends React.PureComponent {
  ...
  _renderMonthName = () => {
    const { month, moment, monthsLocale, monthNameMode, days, weekDaysLocale } =
      this.props;
    let monthName = this.momentPrefix
      ? moment(days[7].date).format(`${this.momentPrefix}MMMM`)
      : monthsLocale[days[7].date.getMonth()];
    let year = this.momentPrefix
      ? moment(days[7].date).format(`${this.momentPrefix}YYYY`)
      : days[7].date.getFullYear();
    let monthHeader = idx(month, (_) => _.header.monthHeader)
    ...
    if (monthNameMode === 'simple' || monthNameMode === 'both') {
      return (
        <View
          key={monthName + year}
          style={idx(month, (_) => _.header.headerStyle)}>
          <Text
            style={[
              styles.monthHeader,
              { ...idx(month, (_) => _.header.headerTextStyle) },
            ]}>
            {monthHeader}
          </Text>
        </View>
