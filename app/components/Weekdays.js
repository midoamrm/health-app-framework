...
export const StaticWeekdays = (props) => {
  const {
    ...
    width,
    type,
    staticWeekdaysBackColor,
    staticWeekdaysTextColor,
  } = props;
  const weekDays =
    type === 'jalali'
      ? jalaliWeekdays
  ...
  return (
    <View
      style={[
        styles.weekDayNames,
        staticWeekdaysBackColor && { backgroundColor: staticWeekdaysBackColor },
      ...
      {weekDays.map((dayName, i) => {
        return (
          <View
            key={i}
            style={[
              styles.weekDayNamesItem,
              { width: width / 7, height: width / 7 },
            ]}>
            <Text
              style={[
                { color: '#444' },
                staticWeekdaysTextColor && { color: staticWeekdaysTextColor },
              ]}>
              {dayName}
            </Text>
          </View>
...
export const MonthWeekdays = (props) => {
  ...
  return (
    <View
      key={`month${days[7].date.getMonth()}Weekdays`}
      style={[
        { ...styles.weekDayNames },
        { borderBottomWidth: 0 },
        { ...idx(month, (_) => _.header.weekdaysStyle) },
      ]}>
      {weekDays.map((dayName, i) => {
        return (
          <View
            ...
            <Text style={idx(month, (_) => _.header.weekdaysTextStyle)}>
              {dayName}
            </Text>
          </View>
        );
